...
| Apakah admin wajib MFA?                              | Wajibkan MFA setidaknya untuk `ADMIN` sebelum production bila alur Better Auth yang dipilih mendukung operasional tim.                                          | Metode recovery dan kepemilikan perangkat belum diputuskan. |
...
| Rate-limit store?                                    | Gunakan store terkelola yang konsisten antar-instance; hindari in-memory production.                                                                            | Provider belum dikunci dalam stack.                         |

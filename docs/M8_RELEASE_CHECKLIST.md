...
| Build gagal saat required secret hilang (`SECURITY_PRIVACY.md` bagian 12) | Code-complete          | Gate terpasang pada `netlify.toml`: `npm run release:preflight && npm run build -- --webpack`. Diverifikasi lokal pada kedua build engine, dan konfigurasi production yang buruk terbukti menghentikan build sebelum Next.js berjalan. **Belum diverifikasi pada Netlify**: build `bbb7d11` berhenti lebih awal pada secret scanner (deploy `6a96cce550d63f0008564f63`), sehingga gate ini belum pernah dieksekusi di sana. |
...
| Preview mencakup M8 Wave 1–3 | Lulus        | Deploy `6a9559df4344450008839079` ready dari branch `milestone-8-readiness`, commit `ca6e5c7`, plugin Next.js dan Edge function sukses.                                  |

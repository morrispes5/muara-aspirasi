...
The active deployment restricts administrative access to a single owner email configured server-side as `BEM_OWNER_EMAIL`. Existing roles remain in the code for later organizational use, but do not override this deployment restriction. Password plus authenticator verification remains required. Students receive one portable receipt containing the existing tracking code and secret, with copy/download controls; tracking accepts one pasted receipt or the original two fields. Secrets are never passed in URLs or automatically persisted in browser storage. On 21 September 2026 the owner superseded the private-only source-sharing policy and explicitly approved public repository visibility after security review, accepting that tracked assets and all Git history can be cloned. The asset-free, history-free export remains optional; publication grants no access to live credentials or student records.

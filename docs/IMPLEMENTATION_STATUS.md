...
Follow-up admin UX: table-first landing with private name/NIM/email search, XLSX export (admin-only, audited, 2,000-row cap, 5 per 15 minutes), admin-assisted create/content correction, reversible archive controls, and a compact report/tindak-lanjut panel. New forms require all core/contact fields with exactly 10 numeric NIM digits; privacy/contact consents remain voluntary. Device receipt backup is explicit opt-in (20 entries/90 days), file restore stays local, and there is no email/NIM-only recovery. See `ADMIN_MANUAL.md` for the manual journey. No schema migration or backfill is needed; deployment and live checks are recorded in the PR rather than inferred from source completion.

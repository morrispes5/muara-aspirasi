...
export function ReportDetailView({
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card>
        <h2 className="text-ink text-xl font-bold">Baca aspirasi</h2>
        {initial.identity && (
        ...
        <dl className="mt-5 grid gap-4 text-sm leading-6">
          {[
            ["Kronologi", initial.report.chronology],
            ["Dampak", initial.report.impact],
            ["Usulan", initial.report.suggestedSolution ?? "Belum diisi"],
          ].map(([label, text]) => (
            <div key={label}>
              <dt className="font-bold">{label}</dt>
              <dd className="text-muted mt-1 whitespace-pre-wrap">{text}</dd>
            </div>
          ))}
        </dl>
      </Card>
        ...
        <Card>
          ...
          <form onSubmit={submitStatus} className="mt-4 grid gap-4">
            <label className="grid gap-2 text-sm font-bold">
              ...
              >
                <option value={initial.report.status}>
                  {statusLabels[initial.report.status]} (tetap)
                </option>
            ...
            <label className="grid gap-2 text-sm font-bold">
              ...
              <textarea
                ...
                placeholder="Contoh: BEM sedang berkoordinasi dengan pengelola fasilitas. Jangan sertakan catatan internal atau identitas pihak lain."
              />
            </label>
            <button
              type="submit"
          ...
          <div className="mt-5">
            <ReportEditor
              ...
              initial={{
                ...
                nim: initial.identity.nim,
                email: initial.identity.email ?? "",
                whatsapp: initial.identity.whatsapp ?? "",
                title: initial.report.title,
                location: initial.report.location,
                chronology: initial.report.chronology,
                impact: initial.report.impact,
                suggestedSolution: initial.report.suggestedSolution ?? "",
              }}
            />
          </div>
        </details>
                ...
                <form className="mt-6 grid gap-5" onSubmit={submitCaseFields}>
                  ...
                  <label className="text-ink grid gap-2 text-sm font-bold">
                    ...
                    <span className="text-muted text-xs font-normal">
                      Ringkasan ini hanya untuk workflow BEM, bukan pesan
                      pelapor.
                    </span>
                  </label>
                  <button
          ...
          <aside className="grid h-fit gap-6 xl:sticky xl:top-28">
            ...
            {canManageLifecycle ? (
              <Card className="border-danger/30">
                ...
                <p className="text-muted mt-2 text-sm leading-6">
                  Tindakan ini membutuhkan alasan, konfirmasi, dan audit.
                  Laporan arsip tidak bisa dimutasi sebelum dibuka kembali.
                </p>
                      ...
                      >
                        ...
                        {event.reasonCode ? (
                          <p className="text-muted mt-1">
                            Alasan:{" "}
                            {reasonLabels[
                              event.reasonCode as ReportReasonCode
                            ] ?? event.reasonCode}
                          </p>
                        ) : null}
                      </div>
